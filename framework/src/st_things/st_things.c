...
void get_things_status_cb(things_es_enrollee_state_e state)
{
	...
	switch (state) {
	case ES_STATE_INIT:					//Defualt state
...
int st_things_push_notification_to_cloud(const char *target_uri, st_things_representation_s *resp_rep)
{
	...
	if (!payload) {
		...
		OCRepPayloadSetUri(resp_rep->payload, target_uri);		//Set target uri
		OCRepPayloadSetPropInt(resp_rep->payload, SEC_ATRRIBUTE_PUSH_MESG_MSGID, 0);		//Set message id
		OCRepPayloadSetPropString(resp_rep->payload, SEC_ATRRIBUTE_PUSH_MESG_PROVIDERID, OCGetServerInstanceIDString());	//Set provider id
		...
		push_ret = push_notification_to_cloud(target_uri, resp_rep->payload);		//Send notification detail to cloud
...
int st_things_reset(void)
{
	...
	int result = 0;
	if (1 != (result = things_reset(NULL, RST_AUTO_RESET))) {	//call the things reset api
...
int st_things_notify_observers(const char *resource_uri)
{
	...
	int result = 0;
	if (1 != (result = things_notify_observers(resource_uri))) {		//call the things notify observers api
