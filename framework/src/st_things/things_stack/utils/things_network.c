...
{
	wifi_manager_result_e res = wifi_manager_get_config(&g_homeap_info);	//call the wifi manager api.
...
bool things_network_connect_home_ap(void)
{
	...
	wifi_manager_ap_config_s *connect_config = things_network_get_homeap_config();	//Get home access point information.
	...
	result = wifi_manager_connect_ap(connect_config);	//call wifi manager api to connect home ap.
	...
	if (res == WIFI_MANAGER_FAIL) {
		...
		if (!dm_is_es_complete()) {
			...
			if (g_retry_connect_cnt <= MAX_WIFI_CONNECT_RETRY_COUNT) {
				/* Set this variable to true for calling the api for reconnect
				 * to home AP.
				 */
...
void things_wifi_list_cleanup(void)
{
	if (g_wifi_scan_info != NULL) {
		...
		piter = pdel = g_wifi_scan_info;	// Copy the starting location of list.
		while (piter != NULL) {
			pdel = piter;
...
void things_wifi_scan_done(wifi_manager_scan_info_s **scan_result, int res)
{
	...
	while (wifi_scan_iter != NULL) {
		if ((wifi_scan_iter->ssid != NULL) && (strlen(wifi_scan_iter->ssid)) != 0) {
			...
			p_last_info = pinfo;
			g_wifi_count++;		// Increment global scan list count
		}
		wifi_scan_iter = wifi_scan_iter->next;
